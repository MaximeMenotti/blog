...
function PlaceTeaser({ place }: PlaceTeaserProps) {
  ...
  const { first: placeTitleSplitFirst, second: placeTitleSplitSecond } = halfSplit(place.title);

  return (
    <div
      className="place-teaser restaurant"
      style={{ backgroundImage: `url(${process.env.REACT_APP_BACKEND_URL + place.teaserImage.url})` }}
    >
      <i className="fas fa-utensils" />
      <span className="half-place-title first">{placeTitleSplitFirst}</span>
      <span className="half-place-title second">{placeTitleSplitSecond}</span>
    </div>
  );
}
